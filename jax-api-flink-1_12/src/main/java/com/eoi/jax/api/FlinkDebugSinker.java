...
/**
 * FlinkDebugSinker用于在debug模式下，为当前节点的构造生成一个sink
 * 用户可以选择将某个节点(source/process类型)设置开启debug，系统会自动将这个节点的输出连接到一个sink上
 * 节点的输出可能是流，也可能是批，所以要同时支持对流和批的sink
 * 不同的sink肯定有不同的配置需求，常见的sink可以是kafka, jdbc, log等
 * 用户配置示例如下：
 *{
 *  "id": "ripple",
 *  "entry": "com.xx.xx.xxx.xx.XXXJob",
 *  "config": {
 *    ...
 *  },
 *  "opts": {
 *     "enableDebug": true,
 *     "debugEntry": "xx.xx.xx.xx.FlinkDebugSinker",
 *     "debugConfig": {
 *       ...
 *      }
 *    }
 * }
 * opts.enableDebug 开启对节点`ripple`开启debug
 * opts.debugEntry 指定FlinkDebugSinker的实现类
 * opts.debugConfig 设置opts.debugEntry的配置, 作为configure的map参数传入FlinkDebugSinker的实现类
 */
