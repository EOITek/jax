...
import static com.eoi.jax.flink1_12.job.process.WatermarkJobConfig.WATERMARK_SOURCE_TIME_FIELD;


@Job(
        name = "WatermarkJob",
        display = "时间水位线设置_1.12",
...
public class WatermarkJob implements FlinkProcessJobBuilder<
    ...
    public DataStream<Map<String, Object>> build(
        ...
        WatermarkStrategy ws = getWatermarkStrategy(config);

        return dataStream.assignTimestampsAndWatermarks(ws);
    }

    public static WatermarkStrategy getWatermarkStrategy(WatermarkJobConfig config) throws JobConfigValidationException {
        ...
        long lagBehindTime = config.getLagBehindTime() * 1000;
        long maxGapTime = config.getMaxGapTime() * 1000;
        long maxDeviateTime = config.getMaxDeviateTime() * 1000;

        WatermarkStrategy ws = WatermarkStrategy
                .forGenerator((ctx) -> new WatermarkPeriodicAssigner<Map<String,Object>>(lagBehindTime,maxGapTime,maxDeviateTime))
                .withTimestampAssigner((element, timestamp) -> {
                    if (config.isReuseWatermark() && timestamp > 0) {
                        return timestamp;
                    }

                    long extractedTimestamp = 0L;
                    ...
                    if (WATERMARK_SOURCE_TIME_FIELD.equals(config.getWatermarkSource())) {
                        ...
                        if (dt != null) {
                            extractedTimestamp = dt.getMillis();
                        }
                    } else { // 使用系统时间
        ...
        if (config.getIdlenessTime() > 0) {
            ws = ws.withIdleness(Duration.ofSeconds(config.getIdlenessTime()));
        }
        return ws;
    }
