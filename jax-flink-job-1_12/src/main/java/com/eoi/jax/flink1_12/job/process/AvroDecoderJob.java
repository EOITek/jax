...
@Job(
        name = "AvroDecoderJob",
        display = "解码器(Avro)_1.12",
        description = "根据指定的avro schema（json格式定义），把byte数组按avro格式decode，输出slot 0 为decode成功数据，输出slot 1 为decode失败数据（错误原因添加在error_msg字段） ",
        icon = "AvroDecoderJob.svg",
        doc = "AvroDecoderJob.md",
        category = OperatorCategory.TEXT_ANALYSIS
)
public class AvroDecoderJob implements FlinkProcessJobBuilder<
        DataStream<Map<String, Object>>,
        Tuple2<DataStream<Map<String, Object>>, DataStream<Map<String, Object>>>,
        AvroDecoderJobConfig> {
    ...
    public Tuple2<DataStream<Map<String, Object>>, DataStream<Map<String, Object>>> build(
            ...
            AvroDecoderJobConfig config,
            JobMetaConfig metaConfig) throws Throwable {
        OutputTag<Map<String, Object>> errorTag = new OutputTag<Map<String, Object>>(metaConfig.getJobId() + "AvroDecoderJobError"){};

        SingleOutputStreamOperator<Map<String, Object>> output =
                dataStream.process(new AvroDecoderFunction(config, errorTag)).name(metaConfig.getJobEntry()).uid(metaConfig.getJobId());
    ...
    @Override
    public AvroDecoderJobConfig configure(Map<String, Object> mapConfig) throws Throwable {
        AvroDecoderJobConfig config = new AvroDecoderJobConfig();
