...
public class FlinkContextMainBuilder {
    ...
    public static void buildGlobalJobParameters(StreamExecutionEnvironment streamEnv, FlinkPipelineDescription flinkPipelineDescription) throws Exception {
        ...
        if (StrUtil.isNotEmpty(flinkPipelineDescription.getRuntimeMode())) {
            String runtimeMode = flinkPipelineDescription.getRuntimeMode().toUpperCase();
            if ("AUTOMATIC".equals(runtimeMode)
                    || "STREAMING".equals(runtimeMode)
                    || "BATCH".equals(runtimeMode)) {
                streamEnv.setRuntimeMode(RuntimeExecutionMode.valueOf(runtimeMode));
                System.out.println("set runtimeMode to " + runtimeMode);
            }
        }
    ...
    public static StreamTableEnvironment buildTableEnvironment(StreamExecutionEnvironment streamEnv, FlinkPipelineDescription flinkPipelineDescription) {
        ...
        StreamTableEnvironment tableEnv = CustomStreamTableEnvironmentImpl.create(streamEnv, settings, new TableConfig());
    ...
    public static FlinkEnvironment buildDAG(
        ...
        // 组装DAG
        FlinkEnvironment context = new FlinkEnvironment(streamEnv, tableEnv, new CustomStatementSet((TableEnvironmentInternal)tableEnv));
