...
public class StringDecoderJobConfig extends DecoderJobConfig implements ConfigValidatable, Serializable {
    @Parameter(
            label = "字符编码",
    ...
    @Parameter(
            label = "输出字段",
            description = "如果指定Decoder后内容输出到指定的字段名，如果不指定输出字段则默认输出到@message字段",
            optional = true,
            defaultValue = ""
    )
