    ...
    public Table build(FlinkEnvironment context, DataStream<Map<String, Object>> mapDataStream, StreamToTableJobConfig config, JobMetaConfig metaConfig) throws Throwable {
        ...
        if (config.isEventTimeEnabled()) {
            ...
            /*
                过滤非法数据
            */
            DataStream<Map<String, Object>> filterStream =
                    setAdvanceConfig(mapDataStream.filter(
                            new EventFilter(config.getTimeFieldName(),convertUtil))
        ...
        context.tableEnv.registerTableSource(tableName, mapStreamTableSource);
        return context.tableEnv.scan(tableName);
    }
