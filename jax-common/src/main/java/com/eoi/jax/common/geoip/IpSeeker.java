...
public class IpSeeker {
    ...
    private static String IP_DATA_PATH = "/tmp/qqwry.dat";

    private static class InnerLazyClass {

        static {
            try {
                IP_DATA_PATH = CopyUtil.copyResource(IpSeeker.class, "/qqwry.dat");
            } catch (Exception ignore) {
            }
        }

        static final IpSeeker ipSeeker = new IpSeeker(IP_DATA_PATH);
    }

    public static IpSeeker getSingleInstance() {
        return InnerLazyClass.ipSeeker;
    }

    /**
     * 构造函数
     */
    private IpSeeker(String url) {
     ...
     *
     * @param offset 记录的偏移
     * @return 返回IpLocation结构
     */
    private synchronized IpLocation getIpLocation(long offset) {
        try {
            // 跳过4字节ip
